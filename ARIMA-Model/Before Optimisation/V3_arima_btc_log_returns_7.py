...
for t+7. No rolling history accumulation occurs. This mirrors the
...
#
# 3. The model generates a 7-step forecast and the final step (t+7)
...
#
# Returns:
# - predictions : array containing the predicted log returns at t+7
# - actuals     : array containing the corresponding true log returns at t+7
def lookback_forecast(context_series, eval_series, order, lookback=LOOKBACK):
    """
    Fixed-window 7-step-ahead ARIMA forecasting.
    ...
    For each position i in eval_series:
        ...
        3. Produce a 7-step-ahead forecast and take the final step (t+7).
    ...
    Returns
    -------
    predictions : np.ndarray  -- 7-step-ahead forecasts aligned to eval_series
    actuals     : np.ndarray  -- corresponding true log-return values at t+7
    """
    ...
    for step in range(len(eval_series) - HORIZON):
        ...
        with warnings.catch_warnings():
            ...
            try:
                ...
                # Take the final step of the forecast as the t+7 prediction
...
#       - predicts the log return 7 steps ahead (t+7)
#
...
def save_outputs(full_df, train_len, valid_len, test_pred, test_actual,
    ...
    pred_df.to_csv("arima_test_predictions_t7.csv", index=False)
    ...
    metrics_df.to_csv("arima_metrics_summary_t7.csv", index=False)

    logging.info("Saved test predictions       -> arima_test_predictions_t7.csv")
    logging.info("Saved metrics summary        -> arima_metrics_summary_t7.csv")
...
def arima_forecast():
    ...
    # ---- VALIDATION ----
    print(f"\n{SEP}")
    print(f"  VALIDATION RESULTS  -  ARIMA (t+7 forecast)")
    print(f"  [Before Hyperparameter Optimisation - Fixed Order]")
    print(SEP)
    print(f"  Params               : {order}")
    print(f"  Forecast horizon     : t+7")
    ...
    # ---- TEST ----
    print(f"\n{SEP}")
    print(f"  FINAL TEST RESULTS  -  ARIMA (t+7 forecast)")
    print(f"  [Before Hyperparameter Optimisation - Fixed Order]")
    print(SEP)
    print(f"  Params               : {order}")
    print(f"  Forecast horizon     : t+7")
