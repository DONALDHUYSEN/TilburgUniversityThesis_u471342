...
=======================================
ARIMA model for Bitcoin daily log-return forecasting (t+30 prediction).
...
precede that point, then a 30-step-ahead forecast is made
for t+30. No rolling history accumulation occurs. This mirrors the
...
# Some intermediate print statements were added with help from Claude to monitor runtime progress.
# ---------------------------------------------------------------------------
# This function performs 30-step-ahead forecasting using a rolling
...
#
# 3. The model generates a 30-step forecast and the final step (t+30)
#    is taken as the prediction for that position.
#
...
#
# Returns:
# - predictions : array containing the predicted log returns at t+30
# - actuals     : array containing the corresponding true log returns at t+30
def lookback_forecast(context_series, eval_series, order, lookback=LOOKBACK):
    """
    Fixed-window 30-step-ahead ARIMA forecasting.
    ...
    For each position i in eval_series:
        ...
        3. Produce a 30-step-ahead forecast and take the final step (t+30).
    ...
    Returns
    -------
    predictions : np.ndarray  -- 30-step-ahead forecasts aligned to eval_series
    actuals     : np.ndarray  -- corresponding true log-return values at t+30
    """
    ...
    for step in range(len(eval_series) - HORIZON):
        # Index of the observation we are fitting the window up to
        ...
        # Fit ARIMA on the fixed 100-day window and forecast HORIZON steps ahead
        with warnings.catch_warnings():
            ...
            try:
                ...
                # Take the final step of the forecast as the t+30 prediction
        ...
        # The actual value to compare against is the observation HORIZON steps ahead
        actuals.append(float(full_series.iloc[target_idx + HORIZON - 1]))
...
#       - predicts the log return 30 steps ahead (t+30)
#
...
#       - generate a 30-step-ahead forecast (t+30 log return)
#
...
def save_outputs(full_df, train_len, valid_len, test_pred, test_actual,
    ...
    pred_df.to_csv("arima_test_predictions_t30.csv", index=False)
    ...
    metrics_df.to_csv("arima_metrics_summary_t30.csv", index=False)

    logging.info("Saved test predictions       -> arima_test_predictions_t30.csv")
    logging.info("Saved metrics summary        -> arima_metrics_summary_t30.csv")
...
def arima_forecast():
    ...
    # ---- VALIDATION ----
    print(f"\n{SEP}")
    print(f"  VALIDATION RESULTS  -  ARIMA (t+30 forecast)")
    print(f"  [Before Hyperparameter Optimisation - Fixed Order]")
    print(SEP)
    print(f"  Params               : {order}")
    print(f"  Forecast horizon     : t+30")
    ...
    # ---- TEST ----
    print(f"\n{SEP}")
    print(f"  FINAL TEST RESULTS  -  ARIMA (t+30 forecast)")
    print(f"  [Before Hyperparameter Optimisation - Fixed Order]")
    print(SEP)
    print(f"  Params               : {order}")
    print(f"  Forecast horizon     : t+30")
