...
FORECAST_HORIZON  = 7    # Predict the log return H days ahead (t+H)
...
# With 288 total combinations, setting N_RANDOM_DRAWS = 30 evaluates ~10%.
# V3 change: reduced from 60 to 30 to offset the cost of leakage-free evaluation.
N_RANDOM_DRAWS = 60
...
# For this script, FORECAST_HORIZON = 7, so the tag becomes "t7".
# The tag is used in filenames for datasets, models, scalers,
# and feature column lists.
#
# This keeps outputs from different forecast horizons clearly separated.
# It also prevents files from being overwritten when multiple horizon
# experiments are executed in the same folder.
def horizon_tag() -> str:
...
# Load Bitcoin data and create the t+7 prediction target         (IMPROVED BY CLAUDE Sonnet 4.6)
# ==============================================================================
# This function reads the raw Bitcoin CSV file.
# It sorts rows chronologically and validates the closing price column.
# Daily log returns are calculated from the closing price.
#
# The target is created with log_return.shift(-7).
# This means the model predicts the daily log return observed 7 days ahead,
# not the cumulative return from today to 7 days later.
#
...
# File names include the t7 horizon tag so runs stay separated.
...
# are never used to update model weights.
...
# Actual and predicted t+7 log returns are added, along with the best parameters,
...
# It loads data, creates the t+7 target, selects input columns, and splits data.
