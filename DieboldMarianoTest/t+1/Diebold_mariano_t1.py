...
# 1.  FILE PATHS  –  edit these if your CSVs live elsewhere
FILES = {
    "ARIMA":    "arima_test_predictions.csv",
    "XGBoost":  "xgboost_test_predictions_t1.csv",
    "LSTM":     "lstm_test_predictions_random_search_1.csv",
    "CNN-LSTM": "cnnlstm_test_predictions_after_hyper_t1.csv",
}
...
ACTUAL_COL = {
    ...
    "LSTM":     "actual_log_return_t_plus_1",
    "CNN-LSTM": "actual_log_return_t_plus_1",
}
PRED_COL = {
    "ARIMA":    "predicted_log_return_t_plus_1",
    "XGBoost":  "predicted_log_return",
    "LSTM":     "predicted_log_return_t_plus_1",
    "CNN-LSTM": "predicted_log_return_t_plus_1",
}
...
def dm_test(e1: np.ndarray, e2: np.ndarray, h: int = 1) -> tuple[float, float, str]:
    """
...
def main():
    ...
    print(f"\n{DIVIDER}")
    print("  SIGNIFICANCE TESTING  –  Bitcoin Log-Return Forecasting (t+1)")
    print(f"{DIVIDER}\n")
    ...
    # Diebold-Mariano
    print(DIVIDER)
    print("  DIEBOLD-MARIANO TEST  (HLN small-sample correction, h=1)")
    ...
    for m1, m2 in pairs:
        e1 = errors[m1].values
        e2 = errors[m2].values
        dm_stat, p_val, verdict = dm_test(e1, e2, h=1)
