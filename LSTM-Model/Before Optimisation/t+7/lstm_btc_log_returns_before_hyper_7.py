"""
lstm_btc_log_returns_before_hyper_7.py
────────────────────────────────────────
LSTM model for Bitcoin log-return forecasting at a t+7 horizon.
Online learning variant with a fixed 100-day lookback window.

PURPOSE OF THIS SCRIPT
───────────────────────
This is the "before hyperparameter optimisation" version for a 7-day forecast.
...
can be compared directly with the optimised version.
...
— is kept consistent with lstm_btc_log_returns_before_hyper_1.py so that
results across forecast horizons are directly comparable.

MODEL DESCRIPTION
─────────────────
A Long Short-Term Memory (LSTM) network is used for sequence-to-one
regression, forecasting the 7-day cumulative log return at t+7.
...
TARGET DEFINITION
─────────────────
The t+7 target is the 7-day cumulative log return:

    target_t = log(P_{t+7} / P_t)
             = log(P_{t+7}) - log(P_t)

This is the natural definition for a multi-step horizon: it measures the
total log return an investor would realise by holding from t to t+7.

This differs from the t+1 version, which uses the single-period return
log(P_t / P_{t-1}).  Using a simple .shift(-7) on the daily log return
would give the return only from day t+6 to t+7, which is NOT the
cumulative 7-day return and would be the wrong target for this horizon.

METHODOLOGY (unchanged from the t+1 version)
──────────────────────────────────────────────
...
Phase 3 — Online evaluation (test split):
  ...
  Then, for each observation in the test split:
    1. Predict the t+7 cumulative log return using the last LOOKBACK rows.
...
─────────────────────────────────────────────────────────────
  - FORECAST_HORIZON = 7 instead of 1.
  - The target is the 7-day CUMULATIVE log return: log(P_{t+7} / P_t).
    This is NOT the same as shifting the daily log return by 7 periods.
  - Output CSV file is named lstm_test_predictions_before_hyper_7.csv.
  - All labels and docstrings reflect the t+7 horizon.
...
FORECAST_HORIZON  = 7    # Predict the cumulative log return 7 days ahead (t+7)
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    available strictly before t is used.  This prevents data leakage.
...
# Load Bitcoin data and create the t+7 cumulative prediction target      (IMPROVED BY CLAUDE Sonnet 4.6)
# ==============================================================================
...
#
# Daily log returns are calculated for use as model features.
# The t+7 target is calculated as log(P_t+7) - log(P_t), which represents
# the cumulative 7-day log return rather than a single daily return.
...
def load_data(csv_path: str) -> pd.DataFrame:
    """
    Load the CSV file, compute log returns and the t+7 cumulative target,
    ...
    The t+7 target is the 7-day cumulative log return:
        ...
        target_t = log(P_{t+7}) - log(P_t)

    This is computed directly from the log-price series by shifting the
    log-price forward by FORECAST_HORIZON (7) periods and subtracting
    the current log-price.

    Important: this is NOT the same as shifting the daily log return by 7.
    Shifting the daily log return by 7 would give the return from day t+6
    to day t+7 only, which is a single-period return, not a 7-day return.
    """
    ...
    # Daily log return (used as a feature): r_t = log(P_t / P_{t-1})
    df["log_return"] = np.log(df[PRICE_COLUMN]).diff()

    # 7-day cumulative log return (the TARGET):
    #   target_t = log(P_{t+7}) - log(P_t)
    # Compute log-price, then subtract current from the value 7 rows ahead.
    log_price = np.log(df[PRICE_COLUMN])
    target_col = f"target_log_return_t_plus_{FORECAST_HORIZON}"
    df[target_col] = log_price.shift(-FORECAST_HORIZON) - log_price

    # Drop rows where either the daily return or the target is NaN
...
# ── Metrics (identical to XGBoost and v1v2 scripts) ──────────────(IMPROVED BY CLAUDE Sonnet 4.6)
# ==============================================================================
...
# Calculate directional accuracy for the t+7 return        (IMPROVED BY CLAUDE Sonnet 4.6)
# ==============================================================================
# This function measures whether the model predicts the correct return direction.
# It compares the sign of the predicted 7-day cumulative log return with the
# sign of the actual 7-day cumulative log return.
#
...
# This metric is useful because directional correctness can still be relevant
# even when exact cumulative return values are difficult to predict.
def directional_accuracy(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    """
    Percentage of predictions where the model correctly predicted
    whether the 7-day return would be positive or negative.
    """
    y_true = np.asarray(y_true)
    y_pred = np.asarray(y_pred)
...
# The realised t+7 cumulative return is then used to update the model with
# train_on_batch.
#
...
def online_evaluate(
    ...
    For each observation i in the evaluation set:
      ...
      2. Predict the t+7 cumulative log return.
    ...
    NOTE ON ONLINE LEARNING AT t+7
    ────────────────────────────────
    In a real-world deployment, the actual t+7 return for day t is not known
    until 7 days later.  In this backtesting framework, the realised target
    is treated as immediately available for the train_on_batch update, which
    is consistent with how the t+1 version operates and preserves comparability
    across horizons.  This is a standard simplification in academic backtests.
    ...
    Returns
    -------
    predictions : numpy array of predicted cumulative log returns
    actuals     : numpy array of actual cumulative log returns
    """
    ...
    for i in range(len(X_eval_scaled)):
        ...
        # Step 2: Predict the t+7 cumulative log return
...
# Actual t+7 cumulative log returns and predicted t+7 cumulative log returns
...
def save_outputs(
    ...
    The output format mirrors lstm_btc_log_returns_before_hyper_1.py so that
    results across forecast horizons are directly comparable.
    """
    ...
    # Save to CSV
    output_filename = "lstm_test_predictions_before_hyper_7.csv"
...
# It loads the data, creates the cumulative t+7 target, builds features,
# selects model inputs, and splits the data.
#
...
def lstm_forecast() -> None:
    ...
    Main entry point.  Orchestrates the full pipeline:
      1. Load and preprocess data (including t+7 cumulative target construction)
    ...
    # Step 1: Load data, compute log returns and the t+7 target, build features
    ...
    # Step 5: Log results to the terminal (same format as the t+1 version)
    ...
    # Step 6: Print a formatted results block to the console (same format as t+1 version)
    print("\n" + "=" * 60)
    print("  TRAIN RESULTS  —  LSTM Online Learning (t+7 forecast)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS  —  LSTM Online Learning (t+7 forecast)")
    print("=" * 60)
    print(f"  Fixed params            : {FIXED_PARAMS}")
    print(f"  Lookback window         : {LOOKBACK} (fixed)")
    print(f"  Forecast horizon        : t+{FORECAST_HORIZON}")
