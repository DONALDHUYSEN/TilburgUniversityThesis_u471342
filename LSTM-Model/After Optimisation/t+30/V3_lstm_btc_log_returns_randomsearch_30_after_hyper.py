...
# This function creates a short label based on the forecast horizon.
# For this script, FORECAST_HORIZON = 30, so the tag becomes "t30".
# The tag is used in filenames for datasets, models, scalers,
# and feature column lists.
#
# This keeps outputs from different forecast horizons separated.
...
#
# The target is created with log_return.shift(-30).
# This means the model predicts the daily log return observed 30 days ahead,
# not the cumulative return from today to 30 days later.
#
...
# File names include the t30 horizon tag so runs stay separated.
...
# Actual and predicted t+30 log returns are added, along with the best params,
...
# It loads data, creates the t+30 target, selects input columns, and splits data.
...
def lstm_forecast() -> None:
    ...
    # print a summary of all saved artefacts for this horizon
    tag = horizon_tag()
