"""
cnnlstm_btc_log_returns_before_hyper_30.py
──────────────────────────────────────────
CNN–LSTM model for Bitcoin log-return forecasting at a t+30 horizon.
...
the 30-day-ahead (t+30) log return.
...
Phase 3 — Online evaluation (test split):
  ...
  Then, for each observation in the test split:
    1. Predict t+30 log return using the last LOOKBACK scaled rows.
...
─────────────────────────────────────────────────────────────
  ...
  - The output CSV is renamed to cnnlstm_test_predictions_before_hyper_30.csv.
...
FORECAST_HORIZON  = 30    # Predict the log return 30 days ahead (t+30)
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    available strictly before t+30 is used.  This prevents data leakage.
...
# The t+30 target is created by shifting daily log returns thirty steps backward.
# This predicts the single daily return at t+30, not the cumulative 30-day return.
# Feature engineering is then applied and the dataframe is returned.
def load_data(csv_path: str) -> pd.DataFrame:
    """
    Load the CSV file, compute log returns and the t+30 target,
    ...
    # The t+30 target is the log return thirty periods ahead of each row
...
def online_evaluate(
    ...
    for i in range(len(X_eval_scaled)):
        ...
        # Step 2: Predict the t+30 log return
...
# Actual t+30 log returns and predicted t+30 log returns are added as columns.
#
...
def save_outputs(
    ...
    # Save to CSV
    output_filename = "cnnlstm_test_predictions_before_hyper_30.csv"
...
# It loads the data, creates the t+30 target, builds features, and selects inputs.
...
# Finally, the t+30 test predictions and model settings are saved to CSV.
def cnnlstm_forecast() -> None:
    ...
    print("\n" + "=" * 60)
    print("  TRAIN RESULTS  —  CNN–LSTM Online Learning (t+30 forecast)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS  —  CNN–LSTM Online Learning (t+30 forecast)")
    print("=" * 60)
    print(f"  Fixed params            : {FIXED_PARAMS}")
    print(f"  Lookback window         : {LOOKBACK} (fixed)")
    print(f"  Forecast horizon        : t+{FORECAST_HORIZON}")
