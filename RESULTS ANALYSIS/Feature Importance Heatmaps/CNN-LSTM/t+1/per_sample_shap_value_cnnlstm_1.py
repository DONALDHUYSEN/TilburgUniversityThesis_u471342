...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Bereken per-sample SHAP values voor CNN-LSTM t+1."
    )

    parser.add_argument("--train", default="train_t1.csv")
    parser.add_argument("--test", default="test_t1.csv")
    parser.add_argument("--model", default="cnnlstm_model_t1.keras")
    parser.add_argument("--scaler", default="cnnlstm_scaler_t1.joblib")
    parser.add_argument("--output-dir", default="outputs_cnnlstm_t1")

    # Wordt automatisch overschreven als het model een andere lookback verwacht.
    parser.add_argument("--lookback", type=int, default=100)

    # SHAP kan traag zijn. Verlaag deze waarden als je geheugenproblemen krijgt.
...
def get_model_input_shape(model) -> tuple[int, int]:
    ...
    if len(input_shape) != 3:
        raise ValueError(
            f"Onverwachte model input shape: {input_shape}. "
            "Dit script verwacht een 3D input: samples x lookback x features."
        )
...
def calculate_shap_in_batches(
    ...
    for start in range(0, total, batch_size):
        ...
        # Soms geeft SHAP een extra outputdimensie terug: samples x lookback x features x output
        if values.ndim == 4:
            values = values[..., 0]

        shap_batches.append(values)

        print(f"SHAP batch klaar: {start} t/m {end} van {total}")
...
def save_outputs(
    ...
    raw_name = f"per_sample_shap_values_cnnlstm_{HORIZON}.csv"
    norm_name = f"per_sample_shap_values_normalized_cnnlstm_{HORIZON}.csv"
    abs_norm_name = f"per_sample_abs_shap_values_normalized_cnnlstm_{HORIZON}.csv"
    top_name = f"per_sample_top_features_cnnlstm_{HORIZON}.csv"
    summary_name = f"shap_feature_summary_cnnlstm_{HORIZON}.csv"
    metadata_name = f"per_sample_shap_metadata_cnnlstm_{HORIZON}.json"
    feature_names_file = f"feature_names_cnnlstm_{HORIZON}.json"
    ...
    metadata = {
        ...
        "aggregation_over_timesteps": "sum",
...
def main() -> None:
    ...
    if len(feature_names) != model_num_features:
        raise ValueError(
            ...
            f"Gevonden features: {len(feature_names)}\n"
            f"Model verwacht: {model_num_features}\n"
            "Gebruik eventueel --extra-exclude kolomnaam1 kolomnaam2 als er extra kolommen "
            "uit de feature matrix moeten."
        )
    ...
    # Gebruik train-median om missende waarden consistent te vullen.
    ...
    # SHAP per timestep aggregeren naar per-sample feature importance.
    # Sum behoudt het teken van de bijdrage.
    ...
    print("Klaar. CNN-LSTM SHAP outputs zijn opgeslagen in:", output_dir.resolve())
