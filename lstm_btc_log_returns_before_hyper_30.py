"""
lstm_btc_log_returns_before_hyper_30.py
─────────────────────────────────────────
LSTM model for Bitcoin log-return forecasting at a t+30 horizon.
Online learning variant with a fixed 100-day lookback window.

PURPOSE OF THIS SCRIPT
───────────────────────
This is the "before hyperparameter optimisation" version for a 30-day forecast.
...
regression, forecasting the 30-day cumulative log return at t+30.
...
TARGET DEFINITION
─────────────────
The t+30 target is the 30-day cumulative log return:

    target_t = log(P_{t+30} / P_t)
             = log(P_{t+30}) - log(P_t)

This is the natural definition for a multi-step horizon: it measures the
total log return an investor would realise by holding from t to t+30.

This differs from the t+1 version, which uses the single-period return
log(P_t / P_{t-1}).  Using a simple .shift(-30) on the daily log return
would give the return only from day t+29 to t+30, which is NOT the
cumulative 30-day return and would be the wrong target for this horizon.
...
Phase 3 — Online evaluation (test split):
  ...
  Then, for each observation in the test split:
    1. Predict the t+30 cumulative log return using the last LOOKBACK rows.
...
─────────────────────────────────────────────────────────────
  - FORECAST_HORIZON = 30 instead of 1.
  - The target is the 30-day CUMULATIVE log return: log(P_{t+30}) - log(P_t).
    This is NOT the same as shifting the daily log return by 30 periods.
  - Output CSV file is named lstm_test_predictions_before_hyper_30.csv.
  - All labels and docstrings reflect the t+30 horizon.
...
FORECAST_HORIZON  = 30   # Predict the cumulative log return 30 days ahead (t+30)
...
# Load Bitcoin data and create the t+30 cumulative prediction target   (IMPROVED BY CLAUDE Sonnet 4.6)
# ==============================================================================
...
# The t+30 target is calculated as log(P_t+30) - log(P_t), which represents
# the cumulative 30-day log return rather than a single daily return.
...
def load_data(csv_path: str) -> pd.DataFrame:
    """
    Load the CSV file, compute log returns and the t+30 cumulative target,
    ...
    The t+30 target is the 30-day cumulative log return:
        ...
        target_t = log(P_{t+30}) - log(P_t)

    This is computed directly from the log-price series by shifting the
    log-price forward by FORECAST_HORIZON (30) periods and subtracting
    the current log-price.

    Important: this is NOT the same as shifting the daily log return by 30.
    Shifting the daily log return by 30 would give the return from day t+29
    to day t+30 only, which is a single-period return, not a 30-day return.
    """
    ...
    # 30-day cumulative log return (the TARGET):
    #   target_t = log(P_{t+30}) - log(P_t)
    # Compute log-price, then subtract current from the value 30 rows ahead.
...
# Calculate directional accuracy for the t+30 return           (IMPROVED BY CLAUDE Sonnet 4.6)
# ==============================================================================
# This function measures whether the model predicts the correct return direction.
# It compares the sign of the predicted 30-day cumulative log return with the
# sign of the actual 30-day cumulative log return.
#
...
# Build one prediction sequence from the latest context       (IMPROVED BY CLAUDE Sonnet 4.6)
# ==============================================================================
...
# The realised t+30 cumulative return is then used to update the model with
# train_on_batch.
#
...
def online_evaluate(
    ...
    For each observation i in the evaluation set:
      ...
      2. Predict the t+30 cumulative log return.
    ...
    for i in range(len(X_eval_scaled)):
        ...
        # Step 2: Predict the t+30 cumulative log return
...
# ── Output ─────────────────────────────────────────────────────────────────────
# Save LSTM t+30 test predictions to CSV                (IMPROVED BY CLAUDE Sonnet 4.6)
# ==============================================================================
# This function exports the final test predictions.
# It aligns the prediction rows with the original dataframe’s test section.
# Actual t+30 cumulative log returns and predicted t+30 cumulative log returns
...
def save_outputs(
    ...
    # Save to CSV
    output_filename = "lstm_test_predictions_before_hyper_30.csv"
...
# It loads the data, creates the cumulative t+30 target, builds features,
...
def lstm_forecast() -> None:
    ...
    # Step 1: Load data, compute log returns and the t+30 target, build features
    ...
    print("\n" + "=" * 60)
    print("  TRAIN RESULTS  —  LSTM Online Learning (t+30 forecast)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS  —  LSTM Online Learning (t+30 forecast)")
    print("=" * 60)
    print(f"  Fixed params            : {FIXED_PARAMS}")
    print(f"  Lookback window         : {LOOKBACK} (fixed)")
    print(f"  Forecast horizon        : t+{FORECAST_HORIZON}")
