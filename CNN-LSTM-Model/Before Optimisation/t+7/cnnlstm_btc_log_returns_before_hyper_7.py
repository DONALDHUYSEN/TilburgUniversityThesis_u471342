"""
cnnlstm_btc_log_returns_before_hyper_7.py
──────────────────────────────────────────
CNN–LSTM model for Bitcoin log-return forecasting at a t+7 horizon.
Online learning variant with a fixed 100-day lookback window.

THIS SCRIPT
───────────────────────
This is the "before hyperparameter optimisation" version of the CNN–LSTM
baseline.  It uses one single, manually chosen set of hyperparameters rather
than running any search procedure.  The goal is to establish a baseline that
can be compared directly with the LSTM baseline
(lstm_btc_log_returns_before_hyper_1.py) and with any subsequent optimised
version.
...
— is kept identical to the LSTM baseline so that results are directly
comparable.

MODEL DESCRIPTION
─────────────────
A CNN–LSTM hybrid network is used for sequence-to-one regression, forecasting
the 7-day-ahead (t+7) log return.
...
METHODOLOGY (unchanged from LSTM baseline)
───────────────────────────────────────────
...
Phase 3 — Online evaluation (test split):
  ...
  Then, for each observation in the test split:
    1. Predict t+7 log return using the last LOOKBACK scaled rows.
    2. Record the prediction and the realised return.
    3. Update the model weights via train_on_batch on the new observation.

KEY DIFFERENCES FROM lstm_btc_log_returns_before_hyper_1.py
─────────────────────────────────────────────────────────────
  - The LSTM architecture is replaced with a CNN–LSTM architecture:
      Conv1D → MaxPooling1D → LSTM → Dropout → Dense(1).
  - FIXED_PARAMS contains CNN–LSTM–specific fields (filters, kernel_size,
    pool_size, lstm_units) instead of LSTM-specific fields (n_lstm_layers).
  - The build function is renamed build_cnnlstm().
  - The initial_train() function calls build_cnnlstm() instead of build_lstm().
  - The output CSV is renamed to cnnlstm_test_predictions_before_hyper_7.csv.
...
FORECAST_HORIZON  = 7    # Predict the log return 7 days ahead (t+7)
...
#
#   filters       : number of convolutional filters in the Conv1D layer
#   kernel_size   : size of the 1-D convolution kernel (time steps)
#   pool_size     : down-sampling factor for MaxPooling1D
#   lstm_units    : number of memory units in the LSTM layer
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    available strictly before t+7 is used.  This prevents data leakage.
...
# Load Bitcoin data and create the t+7 prediction target  
# ---------------------------------------------------------------------------
# CLAUDE SONNET 4.6
# Some intermediate print statements were added with help from Claude to monitor runtime progress.
# ---------------------------------------------------------------------------
...
# The t+7 target is created by shifting daily log returns seven steps backward.
# This predicts the single daily return at t+7, not the cumulative 7-day return.
# Feature engineering is then applied and the dataframe is returned.
def load_data(csv_path: str) -> pd.DataFrame:
    """
    Load the CSV file, compute log returns and the t+7 target,
    ...
    # The t+7 target is the log return seven periods ahead of each row
...
# Metrics (identical to LSTM baseline)
# ---------------------------------------------------------------------------
# CLAUDE SONNET 4.6
# Some intermediate print statements were added with help from Claude to monitor runtime progress.
# ---------------------------------------------------------------------------
...
# The output shape works for both Conv1D and LSTM layers.
# If there are not enough rows, empty arrays are returned.
def build_sequences(X: np.ndarray, y: np.ndarray):
    ...
    The resulting shape (N, LOOKBACK, F) is compatible with both LSTM and
    Conv1D layers, so this function is unchanged from the LSTM baseline.
...
# This helper creates a single input sequence for prediction.
...
# This function defines the hybrid CNN-LSTM architecture.
# The Conv1D layer first extracts local temporal patterns from the lookback window.
# Causal padding is used so the convolution does not use future information.
# MaxPooling1D then compresses the temporal representation.
#
# The LSTM layer processes the extracted temporal features for sequence learning.
# Dropout is added for regularisation.
...
def build_cnnlstm(
    ...
    Design notes:
      ...
      - MaxPooling1D compresses the feature map before it enters the LSTM,
        reducing the effective sequence length the LSTM must process.
    ...
    # MaxPooling1D: reduces the temporal dimension, providing translation
    # invariance and compressing the representation for the LSTM
...
# This function builds and trains the CNN-LSTM using the fixed hyperparameters.
...
# This helps reduce overfitting during the initial training phase.
...
def initial_train(
    ...
    params : hyperparameter dictionary (must include filters, kernel_size,
             pool_size, lstm_units, dropout_rate, learning_rate, batch_size)
    ...
    # Build the CNN–LSTM model using the provided hyperparameters
    model = build_cnnlstm(
...
# The loop logic is the same as the LSTM baseline.
def online_evaluate(
    model:            Sequential,
    ...
    This function is identical to the LSTM baseline — the model object is the
    only thing that changes (CNN–LSTM vs pure LSTM); the loop logic is the same.
    ...
    for i in range(len(X_eval_scaled)):
        ...
        # Step 2: Predict the t+7 log return
...
# This keeps scaling consistent across the forecasting pipeline.
...
# Actual t+7 log returns and predicted t+7 log returns are added as columns.
#
# The fixed lookback window and fixed baseline hyperparameters are also saved.
# This makes the output traceable and comparable with the LSTM baseline.
...
def save_outputs(
    ...
    The output format mirrors the LSTM baseline so that results can be compared
    directly.  The file is renamed to reflect the CNN–LSTM architecture and the
    "before hyperparameter optimisation" baseline status.

    No grid search results CSV is saved here because no search was performed.
    """
    ...
    # Save to CSV
    output_filename = "cnnlstm_test_predictions_before_hyper_7.csv"
...
# It loads the data, creates the t+7 target, builds features, and selects inputs.
# It then splits the data chronologically into train, validation, and test.
#
# The fixed CNN-LSTM configuration is evaluated without hyperparameter search.
# Results are calculated for both the training split and final test split.
# The metrics are logged and printed in a formatted terminal output.
# Finally, the t+7 test predictions and model settings are saved to CSV.
def cnnlstm_forecast() -> None:
    """
    ...
    # Step 5: Log results to the terminal (same format as LSTM baseline)
    ...
    # Step 6: Print a formatted results block to the console (same format as LSTM baseline)
    print("\n" + "=" * 60)
    print("  TRAIN RESULTS  —  CNN–LSTM Online Learning (t+7 forecast)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS  —  CNN–LSTM Online Learning (t+7 forecast)")
    print("=" * 60)
    print(f"  Fixed params            : {FIXED_PARAMS}")
    print(f"  Lookback window         : {LOOKBACK} (fixed)")
    print(f"  Forecast horizon        : t+{FORECAST_HORIZON}")
