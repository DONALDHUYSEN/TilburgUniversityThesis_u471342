"""
cnnlstm_btc_log_returns_before_hyper_1.py
──────────────────────────────────────────
CNN–LSTM model for Bitcoin log-return forecasting at a t+1 horizon.
...
the next-period (t+1) log return.
...
─────────────────────────────────────────────────────────────
  ...
  - The output CSV is renamed to cnnlstm_test_predictions_before_hyper_1.csv.
...
def save_outputs(
    ...
    # Save to CSV
    output_filename = "cnnlstm_test_predictions_before_hyper_1.csv"
...
# It loads the data, creates the t+1 target, builds features, and selects inputs.
...
# Finally, the t+1 test predictions and model settings are saved to CSV.
def cnnlstm_forecast() -> None:
    ...
    print("\n" + "=" * 60)
    print("  TRAIN RESULTS  —  CNN–LSTM Online Learning (t+1 forecast)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("  FINAL TEST RESULTS  —  CNN–LSTM Online Learning (t+1 forecast)")
    print("=" * 60)
    print(f"  Fixed params            : {FIXED_PARAMS}")
    print(f"  Lookback window         : {LOOKBACK} (fixed)")
    print(f"  Forecast horizon        : t+{FORECAST_HORIZON}")
